...
class SpectralMesh:
    def __init__(self,
        ...
        if isinstance(for_mesh_build, str):
            surf_gii = nilearn.surface.load_surf_mesh(for_mesh_build)
            self.mesh_tri = trimesh.Trimesh(vertices=surf_gii.coordinates, faces=surf_gii.faces)
    ...
    def store_data(self,
        ...
        if not is_template:
            ...
            #with tables.open_file(os.path.join(target_f_p, 'spec_coeff.h5'), mode='w') as handle:
             #   c = np.asarray(R @ (self.graph.eig_vecs.T @ self.mesh_tri.vertices))
             #   content_c = handle.create_carray(
             #       handle.root,
              #      'c',
              #      tables.Atom.from_dtype(c.dtype),
